...
do
	...
	do
		...
		then
			...
			echo $file not found in src/CMakeLists.txt
			exitcode=1
		fi
	done
done
...
output=$(grep -rn 'DISABLED_' test | grep -v '#[0-9]\+' | sed 's/^\([a-zA-Z/._]*:[0-9]*\).*/\1  Disabled tests should be documented with their issue number (e.g. \/* #123 *\/)/')
if [ ! -z "$output" ]; then
	echo "$output"
	exitcode=1
fi
...
output=$(grep -rn '^TEST(' test | sed 's/^\([a-zA-Z/._]*:[0-9]*\).*/\1  TEST() should not be used as it does not clean up global state./')
if [ ! -z "$output" ]; then
	echo "$output"
	exitcode=1
fi
...
output=$(grep -rEn '#include <random>|std::random|std::[a-z_]+_engine|std::[a-z_]+_distribution' test | grep -v std::random_access_iterator_tag | sed 's/^\([a-zA-Z/._]*:[0-9]*\).*/\1  Tests should not include any type of random behavior./')
if [ ! -z "$output" ]; then
	echo "$output"
	exitcode=1
fi
...
output=$(find src test -type f -exec dos2unix -ic {} +)
if [[ ${output} ]]; then
	for file in ${output}
	do
